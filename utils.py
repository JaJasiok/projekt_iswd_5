...
def F1(output: torch.FloatTensor, target: torch.LongTensor) -> torch.FloatTensor:
    ...
    Returns:
        float: The F1 score of the model output compared to the target tensor.
    """
    output = (output > 0) * 1
    return f1_score(target.detach().numpy(), output.detach().numpy())
...
class ScoreTracker:
    ...
    def add(self, outputs: torch.FloatTensor, labels: torch.LongTensor) -> None:
        """Calculate and append the loss, auc, acc and f1 scores for the given model outputs
